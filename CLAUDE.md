...
- **Null UI singletons**: `NGame.Instance`, `NEventRoom.Instance`, `NCombatRoom.Instance` etc. are null headless. When game logic dereferences one unguarded (event options, monster death hooks), the option throws halfway and the event loops. Fix it with a targeted Harmony patch (`HeadlessUiPatches.cs` transpilers, or `PatchCosmeticNoOp` for purely visual methods), not a flow special case. Missing GodotStubs members show up as `MissingMethodException` / `TypeLoadException` ("turn loop died"). CoreCLR resolves tokens when it JIT-compiles a method, so one missing member breaks the whole calling method even inside an untaken branch.
- **GodotStubs audit**: `dotnet run --project scripts/godot_stub_audit -- [--reference <real GodotSharp.dll>]` lists GodotSharp members sts2.dll references that `src/GodotStubs` lacks, split into gameplay-reachable and UI-only. After a game update, keep the gameplay list empty (`tests/test_godot_stubs.py` enforces it). New members go in `src/GodotStubs/AuditedMembers.cs` with the real signature (enums are `long`-backed; `Variant` converts out via explicit operators).
