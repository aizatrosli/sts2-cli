#!/usr/bin/env bash
# verify_godot_stubs.sh — the GodotStubs verification gate in one go: build, stub audit and
# value-type diff against the game's GodotSharp.dll, stub tests, full runs, manual-flow runs,
# and a trajectory comparison against a build without the stub changes.
...
#     --base REV             build to record base trajectories with (default 15669bf, the last commit
#                            before the reachable-ui stubs; pass the previous stub commit after later changes)
#
